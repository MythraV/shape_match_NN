...
def train_unet(train_path, val_path, output_dir):
    ...
    for epoch in range(EPOCHS):
        ...
        if val_loss < best_loss:
            ...
            # Save to Drive immediately if output_dir is provided
            if output_dir:
                drive_path = os.path.join(output_dir, "siamese_unet_best.pth")
                torch.save(model.state_dict(), drive_path)
                print(f"Saved Best Model to {drive_path} (Val: {best_loss:.4f})")
            else:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--output_dir', type=str, default=None, help='Directory to save checkpoints (e.g. Google Drive)')
